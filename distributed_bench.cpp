...
int main(int argc, const char *argv[]) {
  if (argc != 7) {
    std::cerr << "Usage ./DistributedBench input-points queries "
                 "ground-truth-file num_neighbors partition-file router-file"
              << std::endl;
    std::abort();
  }
  ...
  MPI_Barrier(MPI_COMM_WORLD); // TODO is this necessary? the subsequent
                               // MPI_Reduce should enforce a sync
