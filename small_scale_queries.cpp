...
int main(int argc, const char *argv[]) {
  if (argc != 8) {
    std::cerr
        << "Usage ./SmallScaleQueries input-points queries ground-truth-file "
           "num-neighbors partition part-method out-file"
        << std::endl;
    std::abort();
  }
  ...
  if (std::filesystem::exists(ground_truth_file)) {
    ...
    std::cout << "wrote ground truth to file " << ground_truth_file
              << std::endl;
  }
  ...
  if (!std::filesystem::exists(partition_file) || part_method == "None") {
...
#endif
    hnswlib::HierarchicalNSW<float> hnsw(&space, points.n, hnsw_parameters.M,
                                         hnsw_parameters.ef_construction, 555);
    parlay::parallel_for(
        0, points.n, [&](size_t i) { hnsw.addPoint(points.GetPoint(i), i); });
    std::cout << "Building HNSW took " << timer.Stop() << " seconds."
              << std::endl;
    ...
    for (int ef : {20, 50, 80, 100, 120, 150, 200, 300, 400}) {
      ...
      double recall =
          Recall(neighbors, distance_to_kth_neighbor, num_neighbors);
      std::cout << "HNSW query with ef = " << ef << " took " << time
                << " seconds. recall = " << recall
                << ". avg latency = " << 1000.0 * time / queries.n << " ms."
                << " avg dist comps "
                << static_cast<double>(hnsw.metric_distance_computations) /
                       queries.n
                << std::endl;
    }
  ...
  Timer timer;
  timer.Start();
  KMeansTreeRouterOptions options{.num_centroids = 32,
                                  .min_cluster_size = 200,
                                  .budget = 50000,
                                  .search_budget = 5000};
  KMeansTreeRouter router;
  router.Train(points, clusters, options);
  std::cout << "Training KMTR took " << timer.Stop() << " seconds."
            << std::endl;
  ...
  std::cout << "Training HNSW router took " << timer.Stop() << " seconds."
            << std::endl;

  std::vector<std::tuple<std::string /*router*/, double /*routing time*/,
                         std::vector<std::vector<int>> /*probes*/>>
      probes_v;

  std::vector<std::vector<int>> buckets_to_probe_kmtr(queries.n),
      buckets_to_probe_hnsw(queries.n);
  timer.Start();
  for (size_t q = 0; q < queries.n; ++q) {
    buckets_to_probe_kmtr[q] =
        router.Query(queries.GetPoint(q), options.search_budget);
  }
  double time = timer.Stop();
  std::cout << "KMTR routing took " << time << " seconds. That's "
            << 1000.0 * time / queries.n << "ms per query, or "
            << queries.n / time << " QPS" << std::endl;
  probes_v.emplace_back(
      std::tuple("KMTR", time, std::move(buckets_to_probe_kmtr)));

  timer.Start();
  for (size_t q = 0; q < queries.n; ++q) {
    buckets_to_probe_hnsw[q] =
        hnsw_router.Query(queries.GetPoint(q), 60).RoutingQuery();
  }
  time = timer.Stop();
  std::cout << "HSNW routing took " << time << " seconds. That's "
            << 1000.0 * time / queries.n << "ms per query, or "
            << queries.n / time << " QPS" << std::endl;
  probes_v.emplace_back(
      std::tuple("HNSW", time, std::move(buckets_to_probe_hnsw)));

  timer.Start();
  InvertedIndexHNSW ivf_hnsw(points);
  ivf_hnsw.hnsw_parameters =
      HNSWParameters{.M = 16, .ef_construction = 200, .ef_search = 120};
  ivf_hnsw.Build(points, clusters);
  std::cout << "Building IVF-HNSW took " << timer.Restart() << " seconds."
            << std::endl;
  ...
  out << "partitioning,routing,shard query,probes,latency,routing latency, "
  ...
  for (const auto &[desc, routing_time, probes] : probes_v) {
    ...
    for (int num_probes = 1; num_probes <= num_shards; ++num_probes) {
      timer.Start();
      for (size_t q = 0; q < queries.n; ++q) {
        auto neighs = ivf.QueryBucket(queries.GetPoint(q), num_neighbors,
                                      probes[q][num_probes - 1]);
      ...
      std::cout << "router = " << desc << " query = IVF "
                << "nprobes = " << num_probes << " recall = " << recall
                << " time = " << time << " avg latency = " << 1000.0 * latency
                << " ms" << std::endl;
      out << part_method << "," << desc << ","
          << "BruteForce"
          << "," << num_probes << "," << latency << ","
          << routing_time / queries.n << "," << time / queries.n << ","
          << recall << std::endl;
    }
    ...
    for (int num_probes = 1; num_probes <= num_shards; ++num_probes) {
      timer.Start();
      for (size_t q = 0; q < queries.n; ++q) {
        auto neighs = ivf_hnsw.QueryBucket(queries.GetPoint(q), num_neighbors,
                                           probes[q][num_probes - 1]);
        neighbors[q].insert(neighbors[q].end(), neighs.begin(), neighs.end());
      }
      time += timer.Stop();
