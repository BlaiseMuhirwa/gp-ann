...
std::string Header() {
  return "approximate,fanout,repetitions,clustersize,degree,graph-recall,"
         "oracle-recall";
}

std::string FormatOutput(const ApproximateKNNGraphBuilder &gb,
                         double oracle_recall, double graph_recall,
                         int degree) {
...
double GraphRecall(const AdjHashGraph &exact_graph,
                   const AdjGraph &approximate_graph) {
  ...
  return static_cast<double>(parlay::reduce(hits)) /
         (approximate_graph.size() * exact_graph[0].size());
}

double FirstShardOracleRecall(const std::vector<NNVec> &ground_truth,
                              const Partition &partition,
                              int num_query_neighbors) {
  ...
  return static_cast<double>(hits) /
         (ground_truth.size() * num_query_neighbors);
}
...
int main(int argc, const char *argv[]) {
  ...
  timer.Start();
  auto exact_graph_hash = parlay::map(
      exact_graph, [num_query_neighbors](const std::vector<int> &neighbors) {
        return std::unordered_set<int>(
            neighbors.begin(),
            neighbors.begin() +
                std::min<int>(num_query_neighbors, neighbors.size()));
      });
  ...
  std::cout << "num configs " << total_num_configs << " num graph builders "
            << graph_builders.size() << std::endl;
  size_t num_gb_configs_processed = 0;
  SpinLock cout_lock;

  timer.Start();
  auto output_lines = parlay::map(
      graph_builders,
      [&](ApproximateKNNGraphBuilder &graph_builder) -> std::string {
        const AdjGraph approximate_graph =
            graph_builder.BuildApproximateNearestNeighborGraph(points,
                                                               max_degree);
        cout_lock.lock();
        num_gb_configs_processed++;
        std::cout << "Num GB configs finished " << num_gb_configs_processed
                  << " / " << graph_builders.size() << std::endl;
        cout_lock.unlock();

        auto outputs = parlay::map(
            num_degree_values,
            [&](int degree) {
              AdjGraph degree_constrained_graph(approximate_graph.size());
              for (size_t i = 0; i < approximate_graph.size(); ++i) {
                degree_constrained_graph[i] = std::vector<int>(
                    approximate_graph[i].begin(),
                    approximate_graph[i].begin() +
                        std::min<int>(approximate_graph[i].size(), degree));
              }

              double graph_recall =
                  GraphRecall(exact_graph_hash, degree_constrained_graph);
              Partition partition = PartitionAdjListGraph(
                  degree_constrained_graph, num_clusters, epsilon,
                  std::min<int>(parlay::num_workers(), 1), true);
              double oracle_recall = FirstShardOracleRecall(
                  ground_truth, partition, num_query_neighbors);
              return "approximate," + FormatOutput(graph_builder, oracle_recall,
                                                   graph_recall, degree);
            },
            1);
  ...
  auto exact_outputs = parlay::map(num_degree_values, [&](int degree) {
    ...
    for (size_t i = 0; i < exact_graph.size(); ++i) {
      degree_constrained_graph[i] =
          std::vector<int>(exact_graph[i].begin(),
                           exact_graph[i].begin() +
                               std::min<int>(exact_graph[i].size(), degree));
    }
    double graph_recall =
        GraphRecall(exact_graph_hash, degree_constrained_graph);
    Partition partition =
        PartitionAdjListGraph(degree_constrained_graph, num_clusters, epsilon,
                              std::min<int>(parlay::num_workers(), 1), true);
    double oracle_recall =
        FirstShardOracleRecall(ground_truth, partition, num_query_neighbors);
