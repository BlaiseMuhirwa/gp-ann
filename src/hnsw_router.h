...
struct HNSWRouter {
  ...
  HNSWRouter(PointSet &routing_points, int num_shards_,
             const std::vector<int> &partition_, HNSWParameters parameters)
      : partition(partition_), num_shards(num_shards_), space(routing_points.d),
  ...
  HNSWRouter(const std::string &file, int dim,
             const std::vector<int> &partition_)
      : partition(partition_), num_shards(NumPartsInPartition(partition_)),
        space(dim), hnsw(new hnswlib::HierarchicalNSW<float>(&space, file)) {
    hnsw->setEf(hnsw_parameters.ef_search);
  }
  ...
  struct ShardPriorities {
    ...
    std::vector<int> SPANNRoutingQuery(double eps) const {
      double closest_shard_dist =
          *std::min_element(min_dist.begin(), min_dist.end()) * (1.0 + eps);
      std::vector<int> probes;
      for (size_t b = 0; b < min_dist.size(); ++b) {
    ...
    std::vector<int> FrequencyQuery() const {
      ...
      std::sort(probes.begin() + 1, probes.end(),
                [&](int l, int r) { return min_dist[l] < min_dist[r]; });
      return probes;
    }
  };
  ...
  ShardPriorities Query(float *Q, int num_voting_neighbors) {
    ...
    while (!near_neighbors.empty()) {
      ...
      near_neighbors.pop();
      result.min_dist[partition[point_id]] =
          std::min(result.min_dist[partition[point_id]], dist);
