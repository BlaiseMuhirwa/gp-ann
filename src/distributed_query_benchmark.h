...
public:
  ...
  void BuildInShardIndex() {
    ...
    hnsw = std::make_unique<hnswlib::HierarchicalNSW<float>>(
        space.get(), shard_points.n, hnsw_parameters.M,
        hnsw_parameters.ef_construction, /* random seed = */ 555);
    parlay::parallel_for(0, shard_points.n, [&](size_t i) {
      hnsw->addPoint(shard_points.GetPoint(i), i);
    });
    hnsw->setEf(hnsw_parameters.ef_search);
    shard_points.Drop(); // TODO we could do the HNSW insert during IO --> halve
  ...
  void ProcessQueries3(const std::vector<int> &query_ids, PointSet &queries) {
    message_queue::FlushStrategy flush_strategy =
        message_queue::FlushStrategy::global;

    struct Request {
      int query_id = -1;
      std::vector<float> coordinates;
    };
    auto merge_requests = [](auto &buf, message_queue::PEID buffer_destination,
    ...
    auto split_requests = [](message_queue::MPIBuffer<float> auto const &buf,
                             message_queue::PEID buffer_origin,
                             message_queue::PEID my_rank) {
    ...
    auto requests_queue = message_queue::make_buffered_queue<Request, float>(
        MPI_COMM_WORLD, merge_requests, split_requests);

    struct Response {
      int query_id;
      std::vector<int> neighbors;
    };
    auto responses_queue = message_queue::make_buffered_queue<Response, int>();
    ...
    // c) get the first requests out as soon as possible so that a machine that
    // runs out of routing
    //    work can get started on retrieving neighbors right away
    // The following performs multiple steps with 2^^i queries routed in step i,
    // followed by sending their messages

    size_t step_size = 128;
    auto qq = parlay::make_slice(query_ids);
    for (size_t i = 0; i < query_ids.size(); i += step_size, step_size *= 2) {
      const auto queries_this_step =
          qq.cut(i, std::min(query_ids.size(), i + step_size));
      auto probes_nested = parlay::map(queries_this_step, [&](int query_id) {
        return Route(queries.GetPoint(query_id));
      });
      // auto probes = parlay::flatten(probes_nested); // no point in doing the
      // zip and flatten in parallel. we have to post messages sequentially
      // anyway
    ...
    auto return_neighbors =
        [&](message_queue::Envelope<Request> auto request_envelope) {
          for (const Request &r : request_envelope.message) {
            int original_sender = request_envelope.sender;
            auto result =
                hnsw->searchKnn(queries.GetPoint(r.query_id), num_neighbors);
            Response response;
    ...
    auto accept_returned_neighbors =
        [&](message_queue::Envelope<Response> auto response_envelope) {
          /* Do nothing here */
          // Dedup?
        };
