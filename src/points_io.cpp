...
PointSet ReadPoints(const std::string &path, int64_t size) {
  ...
  parlay::parallel_for(
      ...
      [&](size_t i) {
        ...
        in.read(reinterpret_cast<char *>(&points.coordinates[begin]),
...
template <typename CoordinateType> PointSet ReadBytes(const std::string &path) {
  uint32_t n, d;
  size_t offset = 0;
  {
    std::ifstream in(path, std::ios::binary);
  ...
  parlay::parallel_for(
      ...
      [&](size_t i) {
        ...
        in.read(reinterpret_cast<char *>(&buffer[0]),
                (end - begin) * sizeof(CoordinateType));
...
PointSet ReadPoints(const std::string &path, int64_t size) {
  if (path.ends_with(".fbin")) {
    ...
    throw std::runtime_error("Invalid file ending for the pointset path. Valid "
                             "options are [.fbin, .u8bin, .i8bin]");
  }
}
...
void WritePoints(PointSet &points, const std::string &path) {
  ...
  out.write(reinterpret_cast<const char *>(&points.coordinates[0]),
            points.coordinates.size() * sizeof(float));
}
...
std::vector<NNVec> ReadGroundTruth(const std::string &path) {
  ...
  std::cout << "num queries = " << num_queries
            << " num neighbors = " << num_neighbors << std::endl;
...
void WriteGroundTruth(const std::string &path,
  ...
  for (uint32_t q = 0; q < num_queries; ++q) {
    for (uint32_t j = 0; j < num_neighbors; ++j) {
      out.write(reinterpret_cast<const char *>(&ground_truth[q][j].second),
                sizeof(uint32_t));
    }
  }
  ...
  for (uint32_t q = 0; q < num_queries; ++q) {
    for (uint32_t j = 0; j < num_neighbors; ++j) {
      out.write(reinterpret_cast<const char *>(&ground_truth[q][j].first),
                sizeof(float));
    }
  }
}
