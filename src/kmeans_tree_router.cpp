...
void KMeansTreeRouter::Train(PointSet &points, const Clusters &clusters,
                             KMeansTreeRouterOptions options) {
  num_shards = clusters.size();
  roots.resize(num_shards);
  dim = points.d;

  std::cout << "Train. num-shards = " << num_shards << " dim = " << dim
            << " budget = " << options.budget << std::endl;
  ...
  parlay::parallel_for(
      0, num_shards,
      [&](int b) {
        // for (int b = 0; b < num_shards; ++b) {      // go sequential for the
        // big datasets on not the biggest memory machines
        PointSet ps = ExtractPointsInBucket(clusters[b], points);
        KMeansTreeRouterOptions recursive_options = options;
        recursive_options.budget =
            double(clusters[b].size() * options.budget) / double(points.n);
...
void KMeansTreeRouter::TrainRecursive(PointSet &points,
                                      KMeansTreeRouterOptions options,
                                      TreeNode &tree_node, int seed) {
  PointSet centroids = RandomSample(
      points, std::max(2, std::min<int>(options.num_centroids, options.budget)),
      seed);
  auto partition = KMeans(points, centroids);
  auto buckets = ConvertPartitionToClusters(partition);
  // std::cout << "num buckets " << buckets.size() << " num centroids " <<
  // centroids.n << " num points " << points.n << " options.budget " <<
  // options.budget
  // << std::endl;

  // check bucket size. stop recursion if small enough. --> partition buckets
  // into those who get a sub-tree and those who don't this is needed for 1-to-1
  // mapping between centroid and sub-tree in the query phase
  ...
  size_t num_buckets_in_recursion = std::distance(
      ...
      std::partition(bucket_size_and_ids.begin(), bucket_size_and_ids.end(),
                     [&](const auto &pair) {
                       return pair.first > options.min_cluster_size;
                     }));
  ...
  parlay::parallel_for(
      ...
      [&](int i) {
        PointSet ps = ExtractPointsInBucket(
            buckets[bucket_size_and_ids[i].second], points);
        KMeansTreeRouterOptions recursive_options = options;
        recursive_options.budget =
            double(bucket_size_and_ids[i].first * options.budget) /
            double(total_size);
...
PointSet KMeansTreeRouter::ReorderCentroids(
    PointSet &centroids, std::vector<std::pair<size_t, size_t>> &permutation) {
  PointSet re;
  re.d = centroids.d;
  re.n = centroids.n;
...
KMeansTreeRouter::FrequencyQueryData
KMeansTreeRouter::FrequencyQuery(float *Q, int budget,
  ...
  for (TreeNode &root : roots) {
    ...
    while (!queue.empty()) {
      ...
      if (u->centroids.coordinates.size() != u->centroids.n * u->centroids.d) {
        std::cout << u->centroids.coordinates.size() << " " << u->centroids.n
                  << " " << u->centroids.d << std::endl;
        throw std::runtime_error("C");
      }
      points.coordinates.insert(points.coordinates.end(),
                                u->centroids.coordinates.begin(),
                                u->centroids.coordinates.end());
