...
Partition RecursiveKMeansPartitioning(PointSet &points, size_t max_cluster_size,
                                      int depth = 0, int num_clusters = -1) {
  if (num_clusters < 0) {
  ...
  //}
  std::cout << "k-means at depth " << depth << " took " << timer.Stop() << " s"
            << std::endl;
  ...
  if (num_overloaded_clusters > 0) {
    std::cout << "At depth " << depth << " there are "
              << num_overloaded_clusters << " / " << num_clusters
              << " too heavy clusters. Refine them" << std::endl;
  }

  int next_part_id = num_clusters;
  for (int part_id = 0; part_id < int(cluster_sizes.size()); ++part_id) {
    if (cluster_sizes[part_id] > max_cluster_size) {
      std::cout << "Cluster " << part_id << " / " << num_clusters
                << " at depth " << depth << " is overloaded "
                << cluster_sizes[part_id] << " / " << max_cluster_size
                << std::endl;

      // Determine nodes in the cluster (could do it for all clusters at once,
      // be we assume that this happens for 1-2 clusters --> this is faster and
      // uses less memory)
      ...
      Partition sub_partition = RecursiveKMeansPartitioning(
          cluster_point_set, max_cluster_size, depth + 1);

      // Translate partition IDs
      int max_sub_part_id =
          *std::max_element(sub_partition.begin(), sub_partition.end());
      std::cout << "Cluster " << part_id << " / " << num_clusters
                << " at depth " << depth << " was overloaded "
                << cluster_sizes[part_id] << " / " << max_cluster_size
                << " and got split into " << max_sub_part_id + 1
                << " sub-clusters" << std::endl;
      for (uint32_t sub_point_id = 0; sub_point_id < cluster.size();
           ++sub_point_id) {
        if (sub_partition[sub_point_id] !=
            0) { // reuse old part ID for the first sub-cluster. the others get
                 // a new one
          uint32_t point_id = cluster[sub_point_id];
          partition[point_id] =
              next_part_id + sub_partition[sub_point_id] -
              1; // the sub_partition IDs used here start with 1 --> -1
        }
      }
...
Partition RebalancingKMeansPartitioning(PointSet &points,
  ...
  std::cout << "There are " << num_overloaded_clusters << " / " << num_clusters
            << " too heavy clusters. Rebalance stuff" << std::endl;
    ...
    while (clusters[c].size() > max_cluster_size) {
      ...
      for (size_t j = 0; j < clusters.size(); ++j) {
        if (clusters[j].size() < max_cluster_size) {
          if (float dist =
                  distance(points.GetPoint(v), centroids.GetPoint(j), points.d);
              dist < min_dist) {
            min_dist = dist;
            target = j;
          }
...
Partition KMeansPartitioning(PointSet &points, int num_clusters,
                             double epsilon) {
...
Partition PartitionGraphWithKaMinPar(CSR &graph, int k, double epsilon,
                                     int num_threads, bool strong, bool quiet) {
  ...
  shm.take_graph(num_nodes, graph.xadj.data(), graph.adjncy.data(),
                 /* vwgt = */ graph.node_weights.empty()
                     ? nullptr
                     : graph.node_weights.data(),
                 /* adjwgt = */ nullptr);
  Timer timer;
  timer.Start();
  ...
  if (!quiet) {
    std::cout << "Partitioning with KaMinPar took " << time << " seconds"
              << std::endl;
  }
  Partition partition(num_nodes);
  for (size_t i = 0; i < partition.size(); ++i) {
    partition[i] = kaminpar_partition[i]; // convert unsigned int partition ID
                                          // to signed int partition ID
  }
  return partition;
}
...
CSR ParallelSymmetrizeAndConvertToCSR(const AdjGraph &adj_graph) {
  ...
  auto degree = parlay::tabulate(adj_graph.size(),
                                 [&](size_t i) -> kaminpar::shm::EdgeID {
  ...
  auto adjncy =
      parlay::sequence<kaminpar::shm::NodeID>::uninitialized(num_edges);
...
Partition PartitionAdjListGraph(const AdjGraph &adj_graph, int num_clusters,
                                double epsilon, int num_threads = 1,
                                bool strong = false, bool quiet = false) {
  CSR csr = ParallelSymmetrizeAndConvertToCSR(adj_graph);
  return PartitionGraphWithKaMinPar(csr, num_clusters, epsilon, num_threads,
                                    strong, quiet);
}

Partition GraphPartitioning(PointSet &points, int num_clusters, double epsilon,
                            bool strong,
                            const std::string &graph_output_path = "") {
  ...
  return PartitionAdjListGraph(knn_graph, num_clusters, epsilon,
                               std::min<int>(64, parlay::num_workers()),
                               strong);
}

Partition PyramidPartitioning(PointSet &points, int num_clusters,
                              double epsilon,
                              const std::string &routing_index_path = "") {
  Timer timer;
  timer.Start();

  // Subsample points
  size_t num_subsample_points =
      10000000; // reasonable value. didn't make much difference
  ...
  PointSet aggregate_points =
      RandomSample(subsample_points, num_aggregate_points, 555);
  ...
  if (!routing_index_path.empty()) {
    ...
    hnswlib::HierarchicalNSW<float> hnsw(&space, aggregate_points.n,
                                         hnsw_parameters.M,
                                         hnsw_parameters.ef_construction, 555);
    parlay::parallel_for(
        0, aggregate_points.n,
        [&](size_t i) { hnsw.addPoint(aggregate_points.GetPoint(i), i); }, 512);
  ...
  ApproximateKNNGraphBuilder graph_builder;
  AdjGraph knn_graph =
      graph_builder.BuildApproximateNearestNeighborGraph(aggregate_points, 10);
  Symmetrize(knn_graph);
  CSR csr = ConvertAdjGraphToCSR(knn_graph);

  // partition
  Partition aggregate_partition = PartitionGraphWithKaMinPar(
      csr, num_clusters, epsilon, std::min<int>(32, parlay::num_workers()),
      false, true);
  WriteMetisPartition(aggregate_partition,
                      routing_index_path + ".routing_index_partition");
  ...
  auto assign_point = [&](size_t i) {
    auto closest_leaders_top_k =
        ClosestLeaders(points, aggregate_points, i, num_leaders);
    ...
    unfinished_points_lock.lock(); // the efficiency of this could be improved,
                                   // but so far it doesn't matter much
    unfinished_points.push_back(i);
    unfinished_points_lock.unlock();
  };

  parlay::parallel_for(0, points.n, assign_point);
  std::cout << "Main Pyramid assignment round finished. "
            << unfinished_points.size() << " still unassigned" << std::endl;

  size_t num_extra_rounds = 0;
  std::vector<uint32_t> frontier;
  while (!unfinished_points.empty()) {
    // now we have to remove the points in aggregated_points associated with
    // overloaded blocks
    std::vector<uint32_t> aggr_points_to_keep;
    Partition new_aggr_partition;
    for (uint32_t i = 0; i < aggregate_partition.size(); ++i) {
      if (num_points_in_cluster[aggregate_partition[i]] <
          max_points_in_cluster) {
        aggr_points_to_keep.push_back(i);
        new_aggr_partition.push_back(aggregate_partition[i]);
      }
    }
    PointSet reduced_aggregate_points =
        ExtractPointsInBucket(aggr_points_to_keep, aggregate_points);
    ...
    parlay::parallel_for(0, frontier.size(),
                         [&](size_t i) { assign_point(frontier[i]); });
    std::cout << "Extra Pyramid assignment round " << ++num_extra_rounds
              << " finished. " << unfinished_points.size()
              << " still unassigned" << std::endl;
  }

  std::cout << "Pyramid partitioning took " << timer.Stop() << " seconds"
            << std::endl;
...
HierarchicalKMeansParlayImpl(PointSet &points, double coarsening_ratio,
                             int depth = 0) {
  ...
  double t = timer.Stop();
  if (depth < 2) {
    std::cout << "KMeans on " << points.n << " points at depth " << depth
              << " with " << level_centroids.n << " / " << num_level_centroids
              << " centroids took " << t << " s." << std::endl;
  }

  if (level_centroids.n == 1) {
    // also stop if we get down to a single centroid. this means we can't split
    // the data any more (for example near-duplicates)
    finished = true;
  }

  if (finished) { // this is weird. it will always aggregate something, even if
                  // points is small...
  ...
  auto recursion_results = parlay::map(
      ...
      [&](const auto &cluster) {
        ...
        return HierarchicalKMeansParlayImpl(cluster_points, coarsening_ratio,
                                            depth + 1);
      },
      depth < 2 ? clusters.size() : 1);

  auto part_id_offsets = parlay::map(recursion_results, [&](const auto &r) {
    return NumPartsInPartition(r.first);
  });
  ...
  auto point_offsets =
      parlay::map(recursion_results, [&](const auto &r) { return r.second.n; });
  ...
  if (centroids_from_recursion.n != num_recursive_clusters) {
    throw std::runtime_error(
        "Num centroids from recursion != num recursive clusters");
  }

  parlay::for_each(
      parlay::zip(clusters, recursion_results, point_offsets, part_id_offsets),
      [&](const auto &z1) {
        const auto &[cluster, recursive_partition_and_points, point_offset,
                     part_id_offset] = z1;

        // remap part IDs
        const auto &recursive_partition = recursive_partition_and_points.first;
        parlay::for_each(
            parlay::zip(recursive_partition, cluster), [&](const auto &z2) {
        ...
        std::memcpy(centroids_from_recursion.GetPoint(point_offset),
                    rec_points.coordinates.data(),
                    rec_points.coordinates.size() * sizeof(float));
      });
...
HierarchicalKMeans(PointSet &points, double coarsening_ratio, int depth = 0) {
  ...
  std::vector<std::pair<Partition, PointSet>> recursion_results(
      clusters.size());
  parlay::parallel_for(
      0, clusters.size(),
      [&](size_t i) {
        if (clusters[i].empty())
          throw std::runtime_error(
              "Cluster points empty. KMeans should remove empty cluster IDs");
        PointSet cluster_points = ExtractPointsInBucket(clusters[i], points);
        recursion_results[i] =
            HierarchicalKMeans(cluster_points, coarsening_ratio, depth + 1);
      },
  ...
  for (size_t i = 0; i < recursion_results.size(); ++i) {
    ...
    if (centroids_from_recursion.n * centroids_from_recursion.d !=
        centroids_from_recursion.coordinates.size()) {
      std::cout << i << " " << centroids_from_recursion.n << " "
                << centroids_from_recursion.d << " "
                << centroids_from_recursion.coordinates.size() << " "
                << rec_points.n << " " << num_rec_parts << " " << depth
                << std::endl;
    ...
    if (num_rec_parts != centroids_from_recursion.n) {
      throw std::runtime_error(
          "Num rec parts doesnt match num centroids from recursion");
    }
  }
...
Partition OurPyramidPartitioning(PointSet &points, int num_clusters,
                                 double epsilon,
                                 const std::string &routing_index_path,
                                 double coarsening_rate = 0.002) {
  std::cout << "Call OurPyramid with coarsening rate " << coarsening_rate
            << std::endl;
  Timer timer;
  timer.Start();
  auto [routing_clusters, routing_points] =
      HierarchicalKMeans(points, coarsening_rate);
  std::cout << "HierKMeans took " << timer.Restart() << std::endl;

  std::cout << "routing_clusters.size() = " << routing_clusters.size()
            << " num routing clusters = "
            << NumPartsInPartition(routing_clusters)
  ...
  hnswlib::HierarchicalNSW<float> hnsw(
      &space, routing_points.n, hnsw_parameters.M,
      hnsw_parameters.ef_construction, /* random seed = */ 500);
  ...
  ApproximateKNNGraphBuilder graph_builder;
  AdjGraph knn_graph =
      graph_builder.BuildApproximateNearestNeighborGraph(routing_points, 20);
  ...
  Partition knn_partition = PartitionGraphWithKaMinPar(
      knn_csr, num_clusters, epsilon, std::min<int>(32, parlay::num_workers()),
      false, true);

  WriteMetisPartition(knn_partition,
                      routing_index_path + ".knn.routing_index_partition");
