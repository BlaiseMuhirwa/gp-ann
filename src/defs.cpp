...
PointSet ExtractPointsInBucket(const std::vector<uint32_t> &bucket,
                               PointSet &points) {
  PointSet ps;
  ps.n = bucket.size();
  ps.d = points.d;
...
ConvertPartitionToClusters(const std::vector<int> &partition) {
...
Cover ConvertClustersToCover(const Clusters &clusters) {
  ...
  for (int cluster_id = 0; cluster_id < static_cast<int>(clusters.size());
       ++cluster_id) {
...
void RemapPartitionIDs(std::vector<int> &partition) {
  ...
  for (int x : partition) {
    num_points_in_shard[x]++;
  }
  if (std::any_of(num_points_in_shard.begin(), num_points_in_shard.end(),
...
TopN ClosestLeaders(PointSet &points, PointSet &leader_points, uint32_t my_id,
                    int k) {
  TopN top_k(k);
