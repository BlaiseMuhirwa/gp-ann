...
std::vector<ShardSearch>
RunInShardSearches(PointSet &points, PointSet &queries,
                   HNSWParameters hnsw_parameters, int num_neighbors,
                   const Clusters &clusters, int num_shards,
                   const std::vector<float> &distance_to_kth_neighbor) {
  std::vector<size_t> ef_search_param_values = {50,  80,  100, 150, 200,
                                                250, 300, 400, 500};
  ...
  for (int b = 0; b < num_shards; ++b) {
    ...
    std::cout << "Start building HNSW for shard " << b << " of size "
    ...
    hnswlib::HierarchicalNSW<float> hnsw(
        &space, cluster.size(), hnsw_parameters.M,
        hnsw_parameters.ef_construction, 555 + b);
    ...
    parlay::parallel_for(
        seq_insertion, cluster.size(),
        [&](size_t i) { hnsw.addPoint(points.GetPoint(cluster[i]), i); }, 512);
    ...
    parlay::execute_with_scheduler(
        std::min<size_t>(32, parlay::num_workers()), [&] {
          ...
          for (const size_t ef_search : ef_search_param_values) {
            parlay::sequence<
                std::priority_queue<std::pair<float, unsigned long>>>
                results(queries.n);
            ...
            parlay::parallel_for(
                ...
                [&](size_t q) {
                  results[q] =
                      hnsw.searchKnn(queries.GetPoint(q), num_neighbors);
                },
                10);
            ...
            double elapsed =
                time_measurements[time_measurements.size() / 2]; // take median

            size_t total_hits = 0;
            parlay::parallel_for(0, queries.n, [&](size_t q) {
              // a not so nice hack, but there is no other way to measure
              // parallel runtime, if we don't want to repeat the query for each
              // probe config (which we don't because it would take forever.
              // this is the parameter tuning code after all.)
              shard_searches[ef_search_param_id].time_query_in_shard[b][q] =
                  elapsed / queries.n;
              ...
              while (!pq.empty()) {
                ...
                if (top.first <= distance_to_kth_neighbor[q]) {
                  hits++;
                  // only need to record a hit... this actually makes things
                  // easier later on
            ...
            std::cout << "Shard search with ef-search = " << ef_search
                      << " total hits " << total_hits << " median time "
                      << elapsed << std::endl;
...
ShardSearch ShardSearch::Deserialize(std::ifstream &in) {
  ...
  s.neighbors.assign(num_shards,
                     std::vector<std::vector<uint32_t>>(num_queries));
  for (int b = 0; b < num_shards; ++b) {
...
void SerializeShardSearches(const std::vector<ShardSearch> &shard_searches,
...
std::vector<ShardSearch>
DeserializeShardSearches(const std::string &input_file) {
  ...
  for (size_t i = 0; i < num_searches; ++i) {
    std::getline(in, header);
    if (header != "S")
      std::cout << "search config doesn't start with marker S. Instead: "
                << header << std::endl;
...
ShardSearch DeserializeOldFormat(std::ifstream &in) {
  ...
  for (int b = 0; b < num_shards; ++b) {
    ...
    while (line_stream >> hits) {
      // old format only stored number of hits, not neighbor ids. this was fine
      // for non-overlapping clusters, but doesn't work with overlap. for the
      // comparisons we make, we are only interested in the number of hits, so
      // to bring the two formats together, just create fake ids for the hits
...
std::vector<ShardSearch>
DeserializeShardSearchesOldFormat(const std::string &input_file) {
  std::ifstream in(input_file);
  size_t num_searches;
  std::string header;
  std::getline(in, header);
