...
void AttributeRecallAndQueryTimeIncreasingNumProbes(
    const RoutingConfig &route, const ShardSearch &search, size_t num_queries,
    size_t num_shards, int num_neighbors,
    std::function<void(EmitResult)> &emit) {
  ...
  for (size_t n_probes = 1; n_probes <= num_shards; ++n_probes) {
    ...
    parlay::parallel_for(0, num_queries, [&](size_t q) {
      ...
      __atomic_fetch_add(
          &total_hits,
          std::min<size_t>(unique_neighbors[q].size(), num_neighbors),
          __ATOMIC_RELAXED);
    });
...
void AttributeRecallAndQueryTimeVariableNumProbes(
  ...
  parlay::parallel_for(0, num_queries, [&](size_t q) {
    ...
    __atomic_fetch_add(&total_num_probes, route.buckets_to_probe[q].size(),
                       __ATOMIC_RELAXED);
  });
  for (size_t q = 0; q < num_queries; ++q) {
    for (int b : route.buckets_to_probe[q]) {
...
void MaxShardSearchRecall(const std::vector<ShardSearch> &shard_searches,
                          int num_neighbors, int num_queries, int num_shards,
                          int num_requested_shards) {
  std::cout << "k = " << num_neighbors << " nq = " << num_queries
            << " num shards = " << num_shards
            << " num requested = " << num_requested_shards << std::endl;
  for (const auto &search : shard_searches) {
    ...
    std::cout << "Search with ef_search = " << search.ef_search << " scored "
              << recall << " total recall" << std::endl;
  }
}

void MaxRoutingRecall(const std::vector<RoutingConfig> &routes,
  ...
  for (const auto &route : routes) {
    ...
    for (size_t q = 0; q < route.buckets_to_probe.size(); ++q) {
      ...
      for (int nprobes = 0;
           nprobes <
           std::min<int>(num_shards, route.buckets_to_probe[q].size());
           ++nprobes) {
        hits[nprobes] += overlap[route.buckets_to_probe[q][nprobes]];
      }
    }

    std::cout << "Route with " << route.index_trainer << " "
              << route.routing_algorithm << " "
              << route.hnsw_num_voting_neighbors << std::endl;
    size_t hit_sum = 0;
...
void PrintCombinationsOfRoutesAndSearches(
    const std::vector<RoutingConfig> &routes,
    const std::vector<ShardSearch> &shard_searches,
    const std::string &output_file, int num_neighbors, int num_queries,
    int num_shards, int num_requested_shards, const std::string &part_method) {

  // std::ofstream out(output_file);
  // header
  std::string header = "partitioning,shard query,routing query,routing "
                       "index,ef-search-shard,num voting points,routing "
                       "time,num probes,recall,QPS,QPS per host,"
                       "QPS without routing, QPS without routing per host,num "
                       "hosts,num shards,requested num shards"
  ...
  for (const auto &route : routes) {
    for (const auto &search : shard_searches) {
      std::function<void(EmitResult)> format_output =
          [&](const EmitResult &r) -> void {
        double recall = static_cast<double>(r.total_hits) /
                        static_cast<double>(num_neighbors * num_queries);
        ...
        // but if a partitioning method already uses more than the requested
        // number of shards then don't give it the load imbalance replicas on
        // top -- this would obfuscate the raw QPS numbers.
        const size_t max_num_hosts =
            std::max(num_requested_shards + 20, num_shards);

        for (; num_hosts <= max_num_hosts; ++num_hosts) {
          const size_t max_shard = std::distance(
              lwr.begin(), std::max_element(lwr.begin(), lwr.end()));
          const double max_latency = lwr[max_shard];

          { // output and formatting bits
            double QPS_without_routing = num_queries / max_latency;
            double QPS_without_routing_per_host =
                QPS_without_routing / num_hosts;
            ...
            str << part_method << ",HNSW," << route.routing_algorithm << ","
                << route.index_trainer << "," << search.ef_search << ","
                << route.hnsw_num_voting_neighbors << ","
                << route.routing_time / num_queries << "," << r.n_probes << ","
                << recall << "," << QPS << "," << QPS_per_host << ","
                << QPS_without_routing << "," << QPS_without_routing_per_host
                << "," << num_hosts << "," << num_shards << ","
                << num_requested_shards << ",";
            str << route.routing_index_options.budget << ","
                << route.routing_index_options.min_cluster_size << ","
                << route.routing_index_options.num_centroids << "\n";

            // out << str.str() << std::flush;
            // std::cout << str.str() << std::flush;
            outputs.push_back(
                Desc{.format_string = str.str(), .recall = recall, .QPS = QPS});
          }
      ...
      if (route.try_increasing_num_shards) {
        AttributeRecallAndQueryTimeIncreasingNumProbes(
            route, search, num_queries, num_shards, num_neighbors,
            format_output);
      } else {
        AttributeRecallAndQueryTimeVariableNumProbes(route, search, num_queries,
                                                     num_shards, num_neighbors,
                                                     format_output);
      }
    }
  }
  ...
  auto dominates = [](const Desc &l, const Desc &r) -> bool {
    return l.recall <= r.recall && l.QPS <= r.QPS;
  };
  ...
  std::sort(pareto.begin(), pareto.end(),
            [](const Desc &l, const Desc &r) { return l.QPS > r.QPS; });
