...
std::vector<float> ComputeDistanceToKthNeighbor(PointSet &points,
                                                PointSet &queries, int k) {
...
std::vector<NNVec> ComputeGroundTruth(PointSet &points, PointSet &queries,
                                      int k) {
...
void OracleRecall(const std::vector<NNVec> &ground_truth,
                  const std::vector<int> &partition, int num_neighbors) {
  ...
  for (int i = 0; i < num_shards && hits[i] > 0; ++i) {
    total += hits[i];
    double recall =
        static_cast<double>(total) / ground_truth.size() / num_neighbors;
    std::cout << "nprobes = " << i + 1 << " oracle recall = " << recall
              << std::endl;
  }
}

/**
 * This function also checks whether the computed distances and order in the
 * ground truth are correct. If not, it will emit a warning and reorder the
 * candidates.
 */
std::vector<float>
ConvertGroundTruthToDistanceToKthNeighbor(std::vector<NNVec> &ground_truth,
                                          int k, PointSet &points,
                                          PointSet &queries) {
  if (ground_truth.size() != queries.n) {
    std::cout << "Ground truth size and number of queries don't match."
              << std::endl;
    std::exit(0);
  }
  std::cout << "Convert ground truth. k = " << k << " points.d " << points.d
            << std::endl;
  ...
  parlay::parallel_for(0, queries.n, [&](size_t q) {
    ...
    auto comp = [](const std::pair<float, uint32_t> &l,
                   const std::pair<float, uint32_t> &r) {
      return l.first < r.first;
    };
    bool is_sorted_before_recalc =
        std::is_sorted(neighs.begin(), neighs.begin() + k, comp);
    if (!is_sorted_before_recalc) {
      __atomic_fetch_add(&wrong_sorts_before_before_recalc, 1,
                         __ATOMIC_RELAXED);
    }
    ...
    if (local_distance_mismatches > 0) {
      __atomic_fetch_add(&distance_mismatches, local_distance_mismatches,
                         __ATOMIC_RELAXED);
    }
  });

  std::cout << distance_mismatches << " out of "
            << ground_truth.size() * ground_truth[0].size()
            << " distances were wrong" << std::endl;
  std::cout << wrong_sorts_before_before_recalc << " out of "
            << ground_truth.size()
            << " neighbors lists were ordered incorrectly before recomputing "
               "distances. And "
            << wrong_sorts << " were ordered incorrectly after recomputing"
            << std::endl;

  for (size_t r = 0; r < epss.size(); ++r) {
    std::cout << "For eps = " << epss[r] << " there were " << wrongss[r]
              << " many distances wrong, i.e., |d1 - d2| > eps" << std::endl;
  }

  return distance_to_kth_neighbor;
}

double Recall(const std::vector<NNVec> &neighbors_per_query,
              const std::vector<float> &distance_to_kth_neighbor, int k) {
  size_t hits = 0;
  ...
  double recall = static_cast<double>(hits) /
                  static_cast<double>(neighbors_per_query.size() * k);
  return recall;
}
