...
inline AdjGraph BuildExactKNNGraph(PointSet &P, int k) {
  AdjGraph graph(P.n);
  parlay::parallel_for(0, P.n,
                       [&](size_t i) { graph[i] = TopKNeighbors(P, i, k); });
  return graph;
}
...
struct ApproximateKNNGraphBuilder {
  ...
  std::vector<Bucket> RecursivelySketch(PointSet &points, const Bucket &ids,
    ...
    // First time around, this is going to extract 950 points out of the entire
    // point set.
    PointSet leader_points = ExtractPoints(points, leaders);

    // NOTE: This I think is going to be the clsuter assignment.
    // Basically, every index i corresponds to a leader and the Bucket is the
    // list of points that are closest to that leader.
  ...
  AdjGraph BuildApproximateNearestNeighborGraph(PointSet &points,
                                                int num_neighbors) {
    ...
    for (int rep = 0; rep < REPETITIONS; ++rep) {
      ...
      std::vector<Bucket> new_buckets =
          RecursivelySketch(points, all_ids, 0, FANOUT);
      if (!quiet)
        std::cout << "Finished sketching rep. It took " << timer2.Stop()
                  << " seconds." << std::endl;
  ...
  std::vector<NNVec> CrunchBucket(PointSet &points, const Bucket &bucket,
                                  int num_neighbors) {
  ...
  AdjGraph BruteForceBuckets(PointSet &points, std::vector<Bucket> &buckets,
                             int num_neighbors) {
    std::vector<SpinLock> locks(points.n);
    std::vector<NNVec> top_neighbors(points.n);

    if (!quiet) {
      std::cout << "Number of buckets to crunch " << buckets.size()
                << std::endl;
    ...
    parlay::parallel_for(
        ...
        [&](size_t bucket_id) {
          ...
          for (size_t j = 0; j < bucket.size(); ++j) {
            ...
            locks[point_id].lock();
            // insert new neighbors. due to possible duplicate neighbors, we
            // can't insert directly into the top-k data structure, and instead
            // have to do this
            auto &n = top_neighbors[point_id];
            n.insert(n.end(), bucket_neighbors[j].begin(),
                     bucket_neighbors[j].end());
            // remove duplicates
            std::sort(n.begin(), n.end(), [](const auto &l, const auto &r) {
              return std::tie(l.second, l.first) < std::tie(r.second, r.first);
            });
            n.erase(std::unique(n.begin(), n.end(),
