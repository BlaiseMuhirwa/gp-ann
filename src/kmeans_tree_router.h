...
public:
  void Train(PointSet &points, const Clusters &clusters,
  ...
  struct FrequencyQueryData {
    ...
    std::vector<int> Query(int num_shards, int num_voting_neighbors) {
      ...
      for (int i = 0;
           i < std::min<int>(num_voting_neighbors, near_neighbors.size());
           ++i) {
  ...
  FrequencyQueryData FrequencyQuery(float *Q, int budget,
...
private:
  ...
  void TrainRecursive(PointSet &points, KMeansTreeRouterOptions options,
                      TreeNode &tree_node, int seed);

  PointSet
  ReorderCentroids(PointSet &centroids,
                   std::vector<std::pair<size_t, size_t>> &permutation);
