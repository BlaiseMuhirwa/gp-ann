...
void NearestCenters(PointSet &P, PointSet &centroids,
                    std::vector<int> &closest_center) {
  parlay::parallel_for(0, P.n, [&](size_t i) {
    closest_center[i] = Top1Neighbor(centroids, P.GetPoint(i));
  });
}

void RemoveEmptyClusters(PointSet &centroids, std::vector<int> &closest_center,
                         const std::vector<size_t> &cluster_size) {
  if (std::any_of(cluster_size.begin(), cluster_size.end(),
                  [](size_t x) { return x == 0; })) {
...
void atomic_fetch_add_float(float *addr, float x) {
  ...
  while (!__atomic_compare_exchange(addr, &expected, &desired, false,
...
void NormalizeCentroidsIP(PointSet &centroids,
                          const std::vector<size_t> &cluster_size,
                          const std::vector<float> &norm_sums) {
...
void SumPointsInClustersIP(PointSet &P, PointSet &centroids,
                           std::vector<int> &closest_center,
                           std::vector<size_t> &cluster_size,
                           const parlay::sequence<float> &vector_sqrt_norms,
                           std::vector<float> &norm_sums, size_t start,
                           size_t end) {
...
void NormalizeCentroidsL2(PointSet &centroids,
                          const std::vector<size_t> &cluster_size) {
  for (size_t c = 0; c < centroids.n; ++c) {
    float *C = centroids.GetPoint(c);
    if (cluster_size[c] == 0) {
...
void SumPointsInClustersL2(PointSet &P, PointSet &centroids,
                           std::vector<int> &closest_center,
                           std::vector<size_t> &cluster_size, size_t start,
                           size_t end) {
  for (size_t i = start; i < end; ++i) {
    int c = closest_center[i];
    cluster_size[c]++;
...
std::vector<size_t> AggregateClusters(
    PointSet &P, PointSet &centroids, std::vector<int> &closest_center,
  ...
  SumPointsInClustersIP(P, centroids, closest_center, cluster_size,
                        vector_sqrt_norms, norm_sums, 0, closest_center.size());
  if (normalize) {
    NormalizeCentroidsIP(centroids, cluster_size, norm_sums);
  }
#else
  SumPointsInClustersL2(P, centroids, closest_center, cluster_size, 0,
                        closest_center.size());
  if (normalize) {
...
std::vector<size_t> AggregateClustersParallel(
    PointSet &P, PointSet &centroids, std::vector<int> &closest_center,
    const parlay::sequence<float> &vector_sqrt_norms, bool normalize = true) {
  size_t block_size =
      std::max<size_t>(5000000, centroids.coordinates.size() * 200);
  if (P.n <= block_size)
    return AggregateClusters(P, centroids, closest_center, vector_sqrt_norms,
                             normalize);
  ...
  // This is what a distributed implementation would do... Not great but at
  // least it can get some speedups
  parlay::internal::sliced_for(
      P.n, block_size, [&](size_t block_id, size_t start, size_t end) {
        ...
        SumPointsInClustersIP(P, b_centroids, closest_center, b_cluster_size,
                              vector_sqrt_norms, b_norm_sums, start, end);
#else
            SumPointsInClustersL2(P, b_centroids, closest_center, b_cluster_size, start, end);
#endif

        for (size_t i = 0; i < cluster_size.size(); ++i) {
          __atomic_fetch_add(&cluster_size[i], b_cluster_size[i],
                             __ATOMIC_RELAXED);
#ifdef MIPS_DISTANCE
...
std::vector<int> KMeans(PointSet &P, PointSet &centroids) {
  ...
  vector_sqrt_norms = parlay::tabulate(P.n, [&](size_t i) -> float {
    return std::sqrt(vec_norm(P.GetPoint(i), P.d));
  });
#endif
...
double ObjectiveValue(PointSet &points, PointSet &centroids,
                      const std::vector<int> &closest_center) {
  return parlay::reduce(
      parlay::delayed_tabulate(points.n, [&](size_t i) -> double {
        return pos_distance(points.GetPoint(i),
                            centroids.GetPoint(closest_center[i]), points.d);
      }));
}

double square(double x) { return x * x; }

std::vector<int> BalancedKMeans(PointSet &points, PointSet &centroids,
                                size_t max_cluster_size) {
  std::vector<int> closest_center = KMeans(points, centroids);

  // precompute norms and sqrts since it slowed down centroid calculation
  parlay::sequence<float> vector_sqrt_norms =
      parlay::tabulate(points.n, [&](size_t i) -> float {
        return std::sqrt(vec_norm(points.GetPoint(i), points.d));
      });

  PointSet cluster_coordinate_sums = centroids;
  std::vector<size_t> cluster_sizes =
      AggregateClustersParallel(points, cluster_coordinate_sums, closest_center,
                                vector_sqrt_norms, false);

  std::cout << "Objective " << ObjectiveValue(points, centroids, closest_center)
            << std::endl;

  auto d_vec_sqrt_norms = parlay::delayed_map(
      vector_sqrt_norms, [](float x) -> double { return x; });
  auto assignment_and_norm = parlay::zip(closest_center, d_vec_sqrt_norms);
  auto cluster_norm_sums =
      parlay::reduce_by_index(assignment_and_norm, centroids.n);
  ...
  std::cout << std::endl;
  std::cout << "total norm sums "
            << std::accumulate(cluster_norm_sums.begin(),
                               cluster_norm_sums.end(), 0.0)
            << std::endl;

  auto is_balanced = [&] {
    return parlay::all_of(cluster_sizes, [&](size_t cluster_size) {
  ...
  auto print_cluster_sizes = [&] {
    ...
    std::cout << "overshot " << overshot << " imbalance " << imbalance
              << " min imb " << imbalance2 << " largest cluster size "
              << max_size << " constraint " << max_cluster_size << std::endl;
  };
  ...
  auto atomic_move = [&](uint32_t point_id, int old_cluster, int new_cluster) {
    ...
    __atomic_fetch_sub(&cluster_sizes[old_cluster], 1,
                       __ATOMIC_RELAXED); // HORRIBLE contention...
...
#ifdef MIPS_DISTANCE
    atomic_fetch_add_double(&cluster_norm_sums[old_cluster],
                            -square(vector_sqrt_norms[point_id]));
    atomic_fetch_add_double(&cluster_norm_sums[new_cluster],
                            square(vector_sqrt_norms[point_id]));
...
// update centroids phase
  ...
  while (round++ <= MAX_ROUNDS) {
    std::cout << "round = " << round << " penalty " << round_penalty
              << std::endl;
    ...
    auto perm = parlay::random_shuffle(parlay::iota<uint32_t>(points.n),
                                       parlay::random(round));
    ...
    for (size_t sub_round = 0; sub_round < num_subrounds; ++sub_round) {
      ...
      parlay::parallel_for(start, end, [&](size_t i) {
        ...
        const float old_cluster_dist =
            pos_distance(centroids.GetPoint(old_cluster), p, points.d);
    ...
    std::cout << "objective " << objective << " next penalty " << next_penalty
              << std::endl;
    if (is_balanced()) {
  ...
  cluster_sizes = AggregateClustersParallel(points, centroids, best_partition,
                                            vector_sqrt_norms, true);
