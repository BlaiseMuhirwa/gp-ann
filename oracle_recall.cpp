    ...
    hits += parlay::reduce(parlay::tabulate(num_queries, [&](size_t q) {
      ...
      for (int j = 0; j < num_neighbors; ++j) {
        ...
        if (!neighbors[q].contains(neighbor) &&
            std::find(cover[neighbor].begin(), cover[neighbor].end(),
                      cluster) != cover[neighbor].end()) {
...
int main(int argc, const char *argv[]) {
  if (argc != 7) {
    std::cerr << "Usage ./OracleRecall ground-truth-file routes-file "
                 "num_neighbors partition-file part-method out-file"
              << std::endl;
    std::abort();
  }
  ...
  auto rrv = parlay::map(routes, [&](const RoutingConfig &route) {
    return RecallForIncreasingProbes(route.buckets_to_probe, cover,
                                     ground_truth, num_neighbors, num_shards);
  });
  ...
  for (size_t j = 0; j < num_shards; ++j) {
    out << part_method << "," << j << "," << rrv[best][j]
        << ",brute-force-shard-search" << std::endl;
  }
  ...
  { // Oracle
    ...
    parlay::parallel_for(0, ground_truth.size(), [&](size_t q) {
      ...
      std::sort(probes.begin(), probes.end(),
                [&](int l, int r) { return freq[l] > freq[r]; });
      buckets_to_probe[q] = std::move(probes);
    });

    auto oracle_recall_values = RecallForIncreasingProbes(
        buckets_to_probe, cover, ground_truth, num_neighbors, num_shards);
    std::cout << "oracle recall. first shard " << oracle_recall_values[0]
              << std::endl;
    for (size_t j = 0; j < num_shards; ++j) {
      out << part_method << "," << j << "," << oracle_recall_values[j]
          << ",oracle" << std::endl;
    }
  }
}
