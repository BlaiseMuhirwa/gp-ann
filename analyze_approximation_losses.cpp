...
std::vector<double>
RecallForIncreasingProbes(const std::vector<std::vector<int>> &buckets_to_probe,
                          const Partition &partition,
                          const std::vector<NNVec> &ground_truth,
                          int num_neighbors, size_t num_shards) {
...
std::vector<std::vector<int>> BruteForceRouting(PointSet &queries,
                                                PointSet &points,
                                                const Partition &partition,
                                                size_t num_shards) {
  ...
  parlay::parallel_for(
      0, queries.n,
      [&](size_t q) {
        std::vector<float> min_dist(num_shards,
                                    std::numeric_limits<float>::max());
        for (size_t i = 0; i < points.n; ++i) {
          if (float dist =
                  distance(points.GetPoint(i), queries.GetPoint(q), points.d);
              dist < min_dist[partition[i]]) {
...
FullDatasetRouting(const std::vector<NNVec> &ground_truth, PointSet &queries,
                   PointSet &points, const Partition &partition,
                   size_t num_shards) {
  std::vector<std::vector<float>> min_dist(
      queries.n,
      std::vector<float>(num_shards, std::numeric_limits<float>::max()));
  ...
  for (size_t q = 0; q < queries.n; ++q) {
    ...
    if (std::any_of(min_dist[q].begin(), min_dist[q].end(), [](float x) {
  ...
  std::cout << non_covered_queries.size()
            << " queries have unconvered shards from the ground-truth set"
            << std::endl;
  ...
  for (size_t q = 0; q < queries.n; ++q) {
    ...
    std::iota(p.begin(), p.end(), 0);
    std::sort(p.begin(), p.end(),
              [&](int l, int r) { return min_dist[q][l] < min_dist[q][r]; });
  }

  return probes;
}

std::vector<std::vector<int>> RouteUsingSingleCenter(PointSet &points,
                                                     PointSet &queries,
...
int main(int argc, const char *argv[]) {
  if (argc != 8) {
    std::cerr
        << "Usage ./AnalyzeApproximationLosses point-file query-file "
           "ground-truth-file num_neighbors partition-file part-method out-file"
        << std::endl;
    std::abort();
  }
  ...
  auto single_center_recall = RecallForIncreasingProbes(
      single_center_probes, partition, ground_truth, num_neighbors, num_shards);
  std::ofstream out2(out_file);
  out2 << "partitioning,num probes,recall,type" << std::endl; // header
  for (size_t j = 0; j < num_shards; ++j) {
    out2 << part_method << "," << j + 1 << "," << single_center_recall[j]
         << ",single center" << std::endl;
    std::cout << part_method << "," << j + 1 << "," << single_center_recall[j]
              << ",single center" << std::endl;
  }
  ...
  timer.Start();
  auto full_probes =
      FullDatasetRouting(ground_truth, queries, points, partition, num_shards);
  std::cout << "Finished full dataset routing. Took " << timer.Stop()
            << std::endl;
  std::vector<double> recall = RecallForIncreasingProbes(
      full_probes, partition, ground_truth, num_neighbors, num_shards);
  std::ofstream out(out_file);
  out << "partitioning,num probes,recall,type" << std::endl; // header
  for (size_t j = 0; j < num_shards; ++j) {
    out << part_method << "," << j + 1 << "," << recall[j] << ",full data"
        << std::endl;
  }
  ...
  std::cout << kmtr_points.n << " " << kmtr_partition.size() << " "
            << num_shards << std::endl;

  timer.Start();
  std::vector<std::vector<int>> kmtr_probes =
      BruteForceRouting(queries, kmtr_points, kmtr_partition, num_shards);
  std::cout << "brute force routing finished. took " << timer.Stop()
            << std::endl;
  recall = RecallForIncreasingProbes(kmtr_probes, partition, ground_truth,
                                     num_neighbors, num_shards);
  std::cout << "Finished KMTR sample brute force routing." << std::endl;

  for (size_t j = 0; j < num_shards; ++j) {
    out << part_method << "," << j + 1 << "," << recall[j] << ",kRt sample"
        << std::endl;
  }
  ...
  auto uf_probes =
      BruteForceRouting(queries, uf_points, uf_partition, num_shards);
  recall = RecallForIncreasingProbes(uf_probes, partition, ground_truth,
                                     num_neighbors, num_shards);

  std::cout << "Finished UF sample brute force routing" << std::endl;

  for (size_t j = 0; j < num_shards; ++j) {
    out << part_method << "," << j + 1 << "," << recall[j] << ",uniform sample"
        << std::endl;
  }
}
