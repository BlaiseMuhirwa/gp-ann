...
int main(int argc, const char *argv[]) {
  if (argc != 9) {
    std::cerr
        << "Usage ./QueryAttribution input-points queries ground-truth-file "
           "num_neighbors partition-file output-file partition_method "
  ...
  std::vector<RoutingConfig> routes = IterateRoutingConfigs(
      ...
      num_neighbors, partition_file + ".routing_index", pyramid_index_file,
      our_pyramid_index_file);
  ...
  PrintCombinationsOfRoutesAndSearches(routes, shard_searches, output_file,
                                       num_neighbors, queries.n, num_shards,
                                       requested_num_shards, part_method);
}
