...
std::vector<int> BalancedKMeansCall(PointSet &points, int k, double eps) {
  ...
  std::cout << "Balanced Kmeans took " << timer.Stop() << " seconds"
            << std::endl;
  return result;
}
...
void PrintImbalance(std::vector<int> &partition, int k) {
  ...
  std::cout << " max part size " << max_part_size << " " << partition.size()
            << " " << k << std::endl;
  double imbalance = double(max_part_size) / (partition.size() / k);
  std::cout << "imbalance " << imbalance << " max part size " << max_part_size
            << " perf balanced " << partition.size() / k << std::endl;
}

int main(int argc, const char *argv[]) {
  if (argc != 6 && argc != 7) {
    std::cerr << "Usage ./Partition input-points output-path num-clusters "
                 "partitioning-method (default|strong) [overlap]"
              << std::endl;
    std::abort();
  }
  ...
  if (part_method == "GP") {
    ...
    partition = PyramidPartitioning(points, k, eps,
                                    part_file + ".pyramid_routing_index");
    ...
    auto rkm = RebalancingKMeansPartitioning(points, max_cluster_size,
                                             adjusted_num_clusters);
    clusters = OverlappingKMeansPartitioningSPANN(points, rkm, k, eps, overlap);
  } else if (part_method == "OurPyramid") {
    partition = OurPyramidPartitioning(
        points, k, eps, part_file + ".our_pyramid_routing_index", 0.02);
    ...
    int adjusted_num_clusters = std::ceil(
        static_cast<double>(num_total_assignments) / max_cluster_size);
    ...
    // leave the same num clusters, since k-means will use more than requested
    // anyways
    Timer timer;
    timer.Start();
    ...
    // but use the original number for the overlap call, so that it chooses the
    // correct max cluster size. The code can handle the case that
    // NumPartsInPartition(bkm) != k
    clusters = OverlappingKMeansPartitioningSPANN(points, bkm, k, eps, overlap);
  } else {
    std::cout << "Unsupported partitioning method " << part_method
              << " . The supported options are [GP, Pyramid, KMeans]"
              << std::endl;
    std::abort();
  }
